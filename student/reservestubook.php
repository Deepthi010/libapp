...
try{
       ...
       $stmt1=$conn->prepare("select count(*) as samebookcount from bookreserve where bookid in
         (select bookid from bookdetails where isbn=? and rollno=?)");
       $stmt1->bindParam(1,$isbn);
       $stmt1->bindParam(2,$rollno);
       $stmt1->execute();
       $row1=$stmt1->fetch(PDO::FETCH_ASSOC);
       $samebookcount=$row1["samebookcount"];
    
       $stmt2=$conn->prepare("select count(*) as bookcount from bookreserve where rollno=?");
       $stmt2->bindParam(1,$rollno);
       $stmt2->execute();
       ...
       if($samebookcount==1){
        ...
        $msg="You have already reserved two books, cannot reserve now";
            ...
            $stmt=$conn->prepare("insert into bookreserve(rollno,reservedate,bookid) values (?,?,?)");
